...
public:
    ...
    const QString&  displayName() const { return _displayName.isEmpty() ? _name : _displayName; }
    const QString&  path()        const { return _path; }
    const QString&  tooltip()     const { return _tooltip; }
    rttr::type      type()        const { return _type; }
    rttr::property  prop()        const { return _prop; }
    bool            isArrayElement() const { return _arrayElement; }
    bool            isLeaf()      const { return _children.isEmpty() && !_expandable; }
    bool            isExpandable() const { return _expandable; }

    void setDisplayName(QString s) { _displayName = std::move(s); }
    void setTooltip(QString s)     { _tooltip     = std::move(s); }
    void setArrayElement(bool v)   { _arrayElement = v; }
    void setExpandable(bool v)     { _expandable   = v; }

    // ── Value state ──────────────────────────────────────────────────────────
    const rttr::variant& liveValue()     const { return _liveValue; }
    const rttr::variant& overrideValue() const { return _overrideValue; }
    const rttr::variant& effectiveValue() const { return _isOverridden ? _overrideValue : _liveValue; }
    ...
    void setOverridden(bool v) { _isOverridden = v; _isDirty = true; }
